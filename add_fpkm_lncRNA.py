...
def read_excle():
    workbook = xlrd.open_workbook('LncRNA Expression Profiling.xlsx')
    ...
    for i in range(25,rowNum,1):
        gene_fpkm = '\t'.join([str(colitem.value) for colitem in sheet2.row(i)[7:19]])
        if i==25:
            gene_fpkms['locus'] = gene_fpkm
	    
        gene_fpkms[sheet2.row(i)[5].value.strip()] = gene_fpkm
...
def write_excel():
   # f = xlwt.Workbook()  # 创建工作簿
    f2 = xlrd.open_workbook(r'All Comparison.xlsx')
    read_excle()
       ...
       # print(sheet1)
        ...
        with open(filename,'w') as f1:
            for i in range(25,rowNum,1):
                ...
                values = gene_fpkms[sheet1.cell(i,4).value]
